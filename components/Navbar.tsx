...
export const Navbar: React.FC<NavbarProps> = ({ onNavigate, onOpenBooking }) => {
  ...
  const pricingLinks = [
    { name: 'Small Business & Enterprise', id: 'business-pricing' },
    { name: 'Residential', id: 'residential-pricing' },
    { name: 'IT Education', id: 'education-pricing' },
  ];

  const BOOKING_URL = 'https://your-booking-link.example.com';
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            >
              ...
              >
                <span>Pricing</span>
                <ChevronDown size={14} className={`transition-transform duration-200 ${isPricingOpen ? 'rotate-180' : ''}`} />
              </button>

              {/* Dropdown Menu */}
              <div 
                className={`absolute top-full left-0 w-64 pt-2 transition-all duration-200 transform origin-top-left ${
                  isPricingOpen ? 'opacity-100 scale-100 visible' : 'opacity-0 scale-95 invisible'
                }`}
              >
                <div className="bg-slate-900 border border-cyan-900/30 rounded-xl shadow-xl overflow-hidden backdrop-blur-xl">
            ...
            {navLinks.map((link) => (
              <button
                key={link.name}
                onClick={(e) => handleLinkClick(e as unknown as React.MouseEvent, link.href)}
                className="text-slate-300 hover:text-cyan-400 transition-colors duration-200 text-sm font-medium uppercase tracking-wide"
              >
                {link.name}
              </button>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden bg-slate-900/95 backdrop-blur-xl border-t border-slate-800 absolute w-full max-h-[80vh] overflow-y-auto">
          <div className="px-4 pt-2 pb-6 space-y-2">
            ...
            <button
              onClick={() => handlePageChange('home')}
              className="block w-full text-left px-3 py-3 text-base font-medium text-slate-300 hover:text-cyan-400 hover:bg-slate-800 rounded-md"
            >
              HOME
            </button>
